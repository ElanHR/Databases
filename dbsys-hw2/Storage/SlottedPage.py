...
class SlottedPageHeader(PageHeader):
  ...
  def slotBufferOffset(self, slotIndex):
    return (math.floor(slotIndex / 8), slotIndex % 8)
  ...
  def freeSlots(self):
    return filter(lambda x: not self.getSlot(x), range(self.numSlots))

  # Returns the slot indexes for all used slots.
  def usedSlots(self):
    return filter(lambda x: self.getSlot(x), range(self.numSlots))
  ...
  def binrepr(cls, buffer):
    ...
    slotArrayLen = math.ceil(numSlots / 8)
    if numSlots > 0:
